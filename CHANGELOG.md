...
### Added
- A Lottie (bodymovin) import path: the header offers a separate "Import Lottie" control next to the existing project import. Selecting a file parses it in memory and opens a report that lists the blockers and the losses with their source paths and next steps **before** anything is applied — Cancel leaves the project untouched, and only "Import and replace project" applies the scene through the same validated path the project import uses. Imported layers keep the shapes, text and images they had: a path becomes a freeform, a rectangle a rect, an ellipse a circle, and text and images their existing KCS types, so an imported scene no longer risks an export refusal caused only by the layer type the importer picked.
