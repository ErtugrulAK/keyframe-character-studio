...
- **Basic IN/OUT Preset UX (M23)**: select a part and configure IN and OUT animation presets from the Inspector Transform tab — Fade, Slide Left/Right/Up/Down, Pop, Spin, each with its own frame duration. This exposes the existing procedural animation engine (`inAnimPreset`/`outAnimPreset` + durations consumed by `computeProceduralDelta`/`applyEditPreset` in edit preview and the broadcast state machine) — **no keyframes are generated, no channels are added, no second animation system**; presets persist through the existing schema (save/reload parity), undo through the shared history, and broadcast stays compatible (the Inspector never touches transport state). Combination presets, user-saved preset editing, multi-select presets and animated gradients remain deferred.
