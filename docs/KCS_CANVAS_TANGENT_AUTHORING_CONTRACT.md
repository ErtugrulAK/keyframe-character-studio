...
- **Extreme-coordinate policy.** Each writer refuses non-finite output instead of storing it: a non-finite pointer result is dropped; a dragged vector whose `Math.hypot` length is not finite leaves the counterpart untouched (its normalized direction would be unusable); a mirror result that is not finite (reachable from individually finite coordinates that overflow the arithmetic, e.g. a counterpart at `±1.7e308`) leaves the counterpart at its previous value. The §8 initializer applies the same rule: a non-finite mirror is replaced by the deterministic direction-and-reach handle, an overflowing span falls back to the documented zero reach, and an overflowing chord falls back to the documented `{x: 1, y: 0}` direction. The guarantee is that **the drag and the smooth toggle never introduce a non-finite value**; a value that already arrived non-finite from imported data (the legacy import path does not sanitize handles) is preserved as-is and is not repaired here.
...
5. **Non-finite refusal.** The initializer never stores a non-finite handle: a mirror that is not finite is dropped in favour of the step 1–4 handle, and a non-finite value already present in the vertex is preserved as it stands (§5).
...
| `src/tests/bezierTangentHandles.test.ts` | 9 | §8 initializer (rings, open/closed, coincident neighbours, partial smooth, mirroring length) plus the non-finite refusal cases (overflowing mirror, overflowing chord) |
