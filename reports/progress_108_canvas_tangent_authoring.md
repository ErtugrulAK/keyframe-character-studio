...
| 5 | MEDIUM — smooth-handle-at-anchor collapse (`Math.hypot(...) || 1`) | **CLOSED** | `withMovedHandle` treats a dragged vector `<= 1e-6` as zero-direction and keeps the counterpart at its own length/direction; a non-finite pointer result is dropped, a non-finite (overflowing) dragged length leaves the counterpart untouched, and a non-finite mirror leaves the counterpart untouched; the §8 initializer refuses a non-finite mirror and an overflowing chord/span the same way. Exact scope of the guarantee: **the drag and the smooth toggle never introduce a non-finite value** — a non-finite value that already arrived through the unsanitized legacy import path is preserved, and the arithmetic is not claimed to be overflow-free. Each case has a test (`1.7e308` counterpart, `1.7e308` dragged vector, `1e308` mirror overflow, overflowing chord). |
...
| Focused per-file counts | eligibility 23 · persistence 6 · overlay 21 · history 3 · initializer 9 · bezierPath 5 · useSerialization 95 |
| `npm test` (full suite) | 108 files / 1638 tests passed |
...
4. **Extreme coordinates are refused, not repaired.** Individually finite coordinates near the double-precision limit can overflow the mirror/normalization arithmetic. Every writer now refuses non-finite *output* (drag: counterpart untouched; initializer: documented degenerate fallback), which is what the tests pin. A non-finite value that a user scene already contains is preserved as-is: the versioned SceneData import sanitizes handles, the legacy `AnimationProject` import does not, and repairing that path is outside this milestone.
...
| 4 | the extreme-coordinate fixes | focused, in progress | — |
