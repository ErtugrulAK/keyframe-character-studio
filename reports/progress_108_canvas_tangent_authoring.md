...
| 5 | MEDIUM — smooth-handle-at-anchor collapse (`Math.hypot(...) || 1`) | **CLOSED** | `withMovedHandle` treats a dragged vector `<= 1e-6` as zero-direction and keeps the counterpart at its own length/direction instead of mirroring a fabricated unit vector; a non-finite pointer result is never written, and a mirror result that overflows to a non-finite value leaves the counterpart unchanged (covered by the `1.7e308` counterpart test). Guarantee stated exactly: **no non-finite value can enter the path** — not that the arithmetic cannot overflow |
...
| Command | Result |
|---|---|
| `npx vitest run` on the tangent/path suites + `useSerialization` | 159 passed (19 files) |
| Focused per-file counts | eligibility 23 · persistence 6 · overlay 20 · history 3 · initializer 7 · bezierPath 5 · useSerialization 95 |
| `npm test` (full suite) | 108 files / 1635 tests passed |
...
| 3 | the review-fix commit (focused, in progress) | — | — |
