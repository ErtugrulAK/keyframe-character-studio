...
export const renderMediaPart = ({ part, fill, overrideMaskShape }: MediaPartProps): React.ReactNode => {
  const isVideo = part.type === 'custom_video';
  const fullW = part.width || (isVideo ? 200 : 180);
  const fullH = part.height || 120;
  const startX = -fullW / 2;
  const startY = -fullH / 2;
  const cornerRadius = part.borderRadius ?? 0;
  ...
  return (
    <g>
      ...
      <rect x={startX} y={startY} width={fullW} height={fullH} rx={cornerRadius} fill="rgba(0,0,0,0.001)" />
      <defs>
        {isGeometricMask && (
          <clipPath id={clipId}>
            {mShape === 'circle' ? (
              ...
              <rect x={startX} y={startY} width={fullW} height={fullH} rx={cornerRadius} />
            )}
          </clipPath>
        )}
      </defs>
        ...
        {isVideo ? (
          part.videoUrl ? (
            <foreignObject x={startX} y={startY} width={fullW} height={fullH}>
              {isYouTube ? (
                <div style={{ width: '100%', height: '100%', overflow: 'hidden', pointerEvents: 'none', position: 'relative', borderRadius: cornerRadius }}>
                  <iframe
                    src={embedUrl}
            ...
            <rect x={startX} y={startY} width={fullW} height={fullH} rx={cornerRadius} fill={fill} />
          )
