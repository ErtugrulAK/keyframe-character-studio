...
function packageAssetDiagnostics(sceneData: SceneData, options: OGrafExportOptions, assets: OGrafAssetPlan[], diagnostics: OGrafExportDiagnostic[]): void {
  for (const layer of sceneData.layers) {
    if (layer.type === 'custom_image' && layer.imageUrl) {
      ...
      if (!entry?.sourcePath || entry.kind !== 'local') {
        diagnostics.push({
          code: entry?.kind === 'external' ? 'OGRAF_EXTERNAL_ASSET_REJECTED' : 'OGRAF_MISSING_ASSET',
          severity: 'ERROR',
          message: `Image asset "${layer.imageUrl}" cannot be packaged without a local sourcePath.`,
          layerId: layer.id,
    ...
    if (layer.type === 'custom_text' && layer.fontFamily) {
      diagnostics.push({
        code: 'OGRAF_FONT_UNVERIFIED',
        severity: 'ERROR',
        message: `Font "${layer.fontFamily}" has no portable local font source in SceneData.`,
        layerId: layer.id,
        layerName: layer.name,
        feature: 'font',
      });
  ...
  for (const asset of assets) {
    if (asset.sourcePath && !isSafePackagePath(asset.packagedPath)) {
...
function generatedFiles(sceneData: SceneData, options: OGrafExportOptions, assets: OGrafAssetPlan[], manifest: OGrafManifest): OGrafPackageFile[] {
  ...
  const imageReferences = Object.fromEntries(assets.map((asset) => [asset.source, asset.packagedPath]));
  const runtime = generateGraphicModule(sceneData, options.publicTextFields, options.publicImageFields, imageReferences);
  return [
    ...
    ...assets.map((asset) => ({ path: asset.packagedPath, kind: 'asset' as const, status: 'planned' as const })),
  ];
}
...
export function compileOGrafPackage(sceneData: SceneData, options: OGrafExportOptions = {}): OGrafGeneratedPackage {
  ...
  packageAssetDiagnostics(sceneData, options, assets, diagnostics);
  ...
  return {
    ...
    files: hasErrors ? [] : generatedFiles(sceneData, options, assets, compiled.manifest),
    assets,
    diagnostics,
  };
}
