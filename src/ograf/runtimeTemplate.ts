...
function text(layer, props, fillOverride) { return '<text x="0" y="0" text-anchor="middle" dominant-baseline="middle" fill="' + escapeXml(fillOverride === undefined ? (layer.fillColor || 'none') : fillOverride) + '" fill-opacity="' + (layer.fillOpacity === undefined ? 1 : layer.fillOpacity) + '" font-size="' + (layer.fontSize || 24) + '" font-family="' + escapeXml(layer.fontFamily || 'sans-serif') + '"' + (props || '') + '>' + escapeXml(layer.textValue || 'TEXT') + '</text>'; }
...
function matteDefs(scene, evaluated) { const defs = []; const ids = Object.create(null); const hidden = Object.create(null); evaluated.forEach((layer) => { const relation = matteRelationship(layer); if (!relation) return; const source = evaluated.find((candidate) => candidate.id === relation.sourceLayerId); if (!source) throw new Error('Missing track matte source: ' + relation.sourceLayerId); const id = runtimeId('kcs-ograf-track-matte', layer.id, source.id, relation.mode, relation.inverted ? 'inverted' : ''); const sourceFill = relation.inverted ? 'black' : (relation.mode === 'luminance' ? escapeXml(source.fillColor || 'white') : 'white'); const sourceBody = content(source, IMAGE_REFERENCES, ' fill="' + sourceFill + '" stroke="none"', sourceFill); const sourceShape = '<g transform="' + transform(scene, source) + '">' + sourceBody + '</g>'; const body = relation.inverted ? '<rect x="0" y="0" width="' + scene.width + '" height="' + scene.height + '" fill="white" />' + sourceShape : sourceShape; defs.push('<mask id="' + id + '" x="0" y="0" width="' + scene.width + '" height="' + scene.height + '" maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" mask-type="' + (relation.inverted ? 'luminance' : relation.mode) + '">' + body + '</mask>'); ids[layer.id] = id; if (!relation.sourceVisible) hidden[relation.sourceLayerId] = true; }); return { defs: defs.join(''), ids, hidden }; }
