...
function fontStyles() { return Object.entries(FONT_REFERENCES).map(([family, path]) => '@font-face{font-family:' + cssString(family) + ';src:url(' + cssString(resourceUrl(path)) + ');}' ).join(''); }
...
function maskDefs(scene, layer) {
  ...
  masks.forEach((mask) => { if (mask.enabled === false) return; const d = maskPathD(layer, mask, scene); if (!d) return;
    ...
    const filter = filterMarkup(id, mask); const filterId = runtimeId(id, 'filter'); const opacity = mask.opacity === undefined ? 1 : mask.opacity;
    let body;
    if (!previousId) { const hole = mode === 'subtract' || mode === 'difference'; body = '<path d="' + escapeXml(hole ? region + ' ' + d : d) + '" fill="white" fill-opacity="' + (hole ? 1 : opacity) + '" fill-rule="' + (hole ? 'evenodd' : 'nonzero') + '" />'; }
    else if (mode === 'add') body = '<rect x="0" y="0" width="' + scene.width + '" height="' + scene.height + '" fill="white" mask="url(#' + previousId + ')" /><path d="' + escapeXml(d) + '" fill="white" fill-opacity="' + opacity + '" />';
    ...
    } else if (mode === 'intersect') body = '<path d="' + escapeXml(d) + '" fill="white" fill-opacity="' + opacity + '" mask="url(#' + previousId + ')" />';
    else body = '<rect x="0" y="0" width="' + scene.width + '" height="' + scene.height + '" fill="white" mask="url(#' + previousId + ')" /><path d="' + escapeXml(d) + '" fill="black" fill-opacity="' + opacity + '" />';
