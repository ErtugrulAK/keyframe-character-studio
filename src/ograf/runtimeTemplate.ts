...
function maskDefs(scene, layer) {
  ...
  const filterMarkup = (id, mask) => { const filterId = runtimeId(id, 'filter'); return mask.feather > 0 || mask.expansion !== 0 ? '<filter id="' + filterId + '" filterUnits="userSpaceOnUse"><feMorphology operator="' + (mask.expansion > 0 ? 'dilate' : 'erode') + '" radius="' + Math.abs(mask.expansion) + '" />' + (mask.feather > 0 ? '<feGaussianBlur stdDeviation="' + (mask.feather / 2) + '" />' : '') + '</filter>' : ''; };
  const flush = () => { if (!addMask || !addPaths.length) return; const id = runtimeId('kcs-ograf-layer-mask', layer.id, addMask.id, 'add'); const filter = filterMarkup(id, addMask); const filterId = runtimeId(id, 'filter'); defs.push(filter + '<mask id="' + id + '" x="0" y="0" width="' + scene.width + '" height="' + scene.height + '" maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" mask-type="alpha"><path d="' + escapeXml(addPaths.join(' ')) + '" fill="white" fill-opacity="' + (addMask.opacity === undefined ? 1 : addMask.opacity) + '"' + (filter ? ' filter="url(#' + filterId + ')"' : '') + ' /></mask>'); ids.push(id); addMask = null; addPaths = []; };
  masks.forEach((mask) => { if (mask.enabled === false) return; const d = maskPathD(layer, mask, scene); if (!d) return; if (mask.mode === 'add' && mask.inverted !== true) { const compatible = addMask && addMask.opacity === mask.opacity && addMask.feather === mask.feather && addMask.expansion === mask.expansion; if (!compatible) flush(); if (!addMask) addMask = mask; addPaths.push(d); return; } flush(); const id = runtimeId('kcs-ograf-layer-mask', layer.id, mask.id); const filter = filterMarkup(id, mask); const filterId = runtimeId(id, 'filter'); const hole = mask.mode === 'subtract' || mask.mode === 'difference' ? mask.inverted !== true : mask.inverted === true; const opacity = mask.opacity === undefined ? 1 : mask.opacity; const path = hole ? region + ' ' + d : d; defs.push(filter + '<mask id="' + id + '" x="0" y="0" width="' + scene.width + '" height="' + scene.height + '" maskUnits="userSpaceOnUse" maskContentUnits="userSpaceOnUse" mask-type="alpha"><path d="' + escapeXml(path) + '" fill="white" fill-opacity="' + (hole ? 1 : opacity) + '" fill-rule="' + (hole ? 'evenodd' : 'nonzero') + '"' + (filter ? ' filter="url(#' + filterId + ')"' : '') + ' />' + (hole && opacity < 1 ? '<path d="' + escapeXml(d) + '" fill="black" fill-opacity="' + (1 - opacity) + '" />' : '') + '</mask>'); ids.push(id); }); flush(); return { defs: defs.join(''), ids }; }
