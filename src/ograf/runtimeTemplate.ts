...
function renderScene(scene, frame, imageReferences) {
  ...
  const markup = evaluated.map((layer) => {
    ...
    let body;
    if (layer.type === 'custom_text') body = '<text x="0" y="0" text-anchor="middle" dominant-baseline="middle" fill="' + escapeXml(layer.fillColor || 'none') + '" fill-opacity="' + (layer.fillOpacity === undefined ? 1 : layer.fillOpacity) + '" stroke="' + escapeXml(layer.strokeColor || 'none') + '" stroke-opacity="' + (layer.strokeOpacity === undefined ? 1 : layer.strokeOpacity) + '" stroke-width="0.5" font-size="' + (layer.fontSize || 24) + '" font-weight="bold" font-family="' + escapeXml(layer.fontFamily || 'Outfit') + '" vector-effect="non-scaling-stroke">' + escapeXml(layer.textValue || 'TEXT') + '</text>';
